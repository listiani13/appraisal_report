...
<script>
	...
	$(document).ready(function() {
		...
		$('#requirementRow').hide();
		$('[name="btnCreateSubmit"]').attr('disabled','disabled');
		...
		$('#plant').on('change', function() {
			...
			$.ajax({
		        ...
		        success: function (response) {
		        	...
		        	$('select[name="nik[]"]').append(response);
		...
		/*$('#direct_superior').on('change', function() {

			alert($('#direct_superior').val());
		});	*/
		var orgchart = new getOrgChart(document.getElementById("strukturOrganisasi"), {			
	        ...
	        dataSource: [
	            ...
	            { id: 3, parentId: 2, Name: " "},
	            { id: 4, parentId: 2, Name: " "},
	            { id: 5, parentId: 2, Name: " "}
	        ]
	    });
	    //('g[class="get-btn"][data-btn-action="add"]').remove();
		//$('[data-node-id="4"] text').text(10);
		/*alert($('[data-node-id="4"] text').text());*/
		...
		$('input[name="job_description[]"]').on('change', function(){
			if($('input[name="job_description[]"]').val() != '' && jobDescValidation()){
				...
				$('#btnUpJobDesc').on('click', function(){
					$('#requirementRow').show();
					$('[name="btnCreateSubmit"]').removeAttr('disabled');
